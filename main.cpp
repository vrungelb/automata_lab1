...
// меню действий над выбранным движком, выходит при выборе "back"
static void engineMenu(Analyz* analyzer, const std::string& name) {
    int option = 0;
    std::string str;
    std::string f;
    std::ofstream out;
...
int main() {
    ...
    try {
        int mainOption = 0;
        while (mainOption != 3 && std::cin.good()) {
            std::cout << "=== Main menu ===" << std::endl;
            std::cout << "1. Choose engine" << std::endl;
            ...
            if (mainOption == 1) {
                ...
                if (engineOption == 1) {
                    ...
                    std::cout << "Invalid engine choice" << std::endl;
                    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                    continue;
                }

                engineMenu(analyzer, name);
                delete analyzer;
